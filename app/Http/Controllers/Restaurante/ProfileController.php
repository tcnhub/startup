...
{
    private function user() { return Auth::guard('restaurante')->user(); }

    public function edit()
    {
        $user = $this->user();
        return view('restaurante.profile.edit', compact('user'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'email'            => ['required', 'email', Rule::unique('restaurantees')->ignore($user->id)],
            'telefono'         => ['nullable', 'string', 'max:50'],
            'direccion'        => ['nullable', 'string', 'max:255'],
            'descripcion'      => ['nullable', 'string'],
            'numero_estrellas' => ['nullable', 'integer', 'min:1', 'max:5'],
            'tipo_restaurante'       => ['nullable', 'string'],
    ...
    {
        return view('restaurante.profile.password');
    }
    ...
    public function updatePassword(Request $request)
    {
        $user = $this->user();
        $request->validate([
            'current_password' => ['required', 'current_password:restaurante'],
    ...
    public function gallery()
    {
        $user = $this->user();
        $photos = $user->photos;
        return view('restaurante.profile.gallery', compact('user', 'photos'));
    }
    ...
    public function uploadPhoto(Request $request)
    {
        ...
        $path = $request->file('photo')->store('restaurante/gallery', 'public');
