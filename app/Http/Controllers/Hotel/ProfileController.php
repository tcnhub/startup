...
{
    private function user() { return Auth::guard('hotel')->user(); }

    public function edit()
    {
        $user = $this->user();
        return view('hotel.profile.edit', compact('user'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'email'            => ['required', 'email', Rule::unique('hoteles')->ignore($user->id)],
            'telefono'         => ['nullable', 'string', 'max:50'],
            'direccion'        => ['nullable', 'string', 'max:255'],
            'descripcion'      => ['nullable', 'string'],
            'numero_estrellas' => ['nullable', 'integer', 'min:1', 'max:5'],
            'tipo_hotel'       => ['nullable', 'string'],
    ...
    {
        return view('hotel.profile.password');
    }
    ...
    public function gallery()
    {
        $user = $this->user();
        $photos = $user->photos;
        return view('hotel.profile.gallery', compact('user', 'photos'));
    }
    ...
    public function uploadPhoto(Request $request)
    {
        ...
        $path = $request->file('photo')->store('hotel/gallery', 'public');
