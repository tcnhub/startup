...
{
    private function user() { return Auth::guard('transportista')->user(); }

    public function edit()
    {
        $user = $this->user();
        return view('transportista.profile.edit', compact('user'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'email'            => ['required', 'email', Rule::unique('transportistaes')->ignore($user->id)],
            'telefono'         => ['nullable', 'string', 'max:50'],
            'direccion'        => ['nullable', 'string', 'max:255'],
            'descripcion'      => ['nullable', 'string'],
            'numero_estrellas' => ['nullable', 'integer', 'min:1', 'max:5'],
            'tipo_transportista'       => ['nullable', 'string'],
    ...
    {
        return view('transportista.profile.password');
    }
    ...
    public function updatePassword(Request $request)
    {
        $user = $this->user();
        $request->validate([
            'current_password' => ['required', 'current_password:transportista'],
    ...
    public function gallery()
    {
        $user = $this->user();
        $photos = $user->photos;
        return view('transportista.profile.gallery', compact('user', 'photos'));
    }
    ...
    public function uploadPhoto(Request $request)
    {
        ...
        $path = $request->file('photo')->store('transportista/gallery', 'public');
