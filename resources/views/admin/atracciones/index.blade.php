...
@section('title', 'Gestión de Atracciones')
...
<div class="container-fluid">
    ...
    <div class="card mt-4 shadow-sm">
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <a href="{{ route('admin.atraccions.create') }}" class="btn btn-primary btn-sm">+ Nuevo</a>
        </div>
        <div class="card-body">
                ...
                <table class="table table-hover align-middle">
                    ...
                    @forelse($records as $i => $record)
                        <tr class="{{ $record->trashed() ? 'table-danger' : '' }}">
                            ...
                            <td>
                                @if($record->trashed())
                                    <a href="{{ route('admin.atraccions.restore', $record->id) }}" class="btn btn-sm btn-outline-success">Restaurar</a>
                                @else
                                    <a href="{{ route('admin.atraccions.show', $record) }}" class="btn btn-sm btn-outline-info">Ver</a>
                                    <a href="{{ route('admin.atraccions.edit', $record) }}" class="btn btn-sm btn-outline-primary">Editar</a>
                                    <form method="POST" action="{{ route('admin.atraccions.destroy', $record) }}" class="d-inline" onsubmit="return confirm('¿Eliminar?')">
                                        @csrf @method('DELETE')
